...
public class U
{
    ...
    public static void disp(AddressStruct addStruct) throws Exception
    {
      ...
      //  U.log(" shatamIndexQueryString:" + addStruct.getshatamIndexQueryString());
    ...
    public static void disp(String address1, String address2, String city, String zip, String state, AddressStruct addStruct)
            throws Exception
    {
    	/*
        U.log("------------- " + addStruct.hitScore + "/" + addStruct._hnDistance + "/" + addStruct.getshatamIndexQueryString());
        U.log("Q: " + address1 + ", " + address2 + ", " + city + ", " + zip + ", " + state);
        U.log("A: " + AbstractIndexType.NORMAL.buildQuery(addStruct));
        String foundStreet = addStruct.toOnlyStreet().toString().toUpperCase();
        U.log("S: " + foundStreet);
        */
    ...
    public static String getSearchableAddress(String pAddress1, String pAddress2) throws Exception
    {
     ...
     //   U.log("AFTER modification:::"+pAddress1+""+pAddress2);
        ...
        if (!StrUtil.isEmpty(address1) && StrUtil.containsOnlyAlphaAndSpaces(address1) && address2.matches("\\d+ .*")
                && StrUtil.containsString(address2))
        {
            //if (address1.contains("diner"))
            {
              //  U.log("*********** " + address1 + " probably is an establishment NAME");
                System.err.println(address1 + " probably is an establishment NAME");
                address1 = "";
            }
        }
        ...
        address = correctNumbersDisguisedAsWords(address);
        ...
        {
            String twochar = StrUtil.extractPattern(address, " ([\\w\\d] [\\w\\d]) ");
            // U.log("00:  twochar :"+twochar );
            //U.log("11:  address:"+address );
            if (!StrUtil.isEmpty(twochar))
            {
                //address = address.replaceFirst(twochar, twochar.replaceAll(" ", ""));
            }
        ...
        // Separate directional words from numbers
        {
            ...
            String hn = StrUtil.extractPattern(address, "(\\d+)" + DIR_REG + " ", 1);
            ...
            if (hn != null)
            {
                address = address.replaceFirst(hn, " " + hn + " ").replaceAll("\\s+", " ");
        ...
        //Fix Rural Route stuff RT #1, BOX 125
        {
            ...
            if (!StrUtil.isEmpty(rrCurrName))
            {
                ...
                //String rrCurrNumb = StrUtil.extractPattern(address, rrPattern, 3);
            }
        ...
        //One Letter Street  fix 857 H ST
        {
            ...
            if (address.matches(pattern))
            {
                String oneLetterStreet = StrUtil.extractPattern(address, pattern, 1);
                //U.log("oneLetterStreet:"+oneLetterStreet);
                if (oneLetterStreet.length() != 1)
                {
                    throw new Exception("oneLetterStreet.len!=1: " + oneLetterStreet);
                } else
                {
                    String sixLetterStreet = oneLetterStreet + oneLetterStreet + oneLetterStreet + oneLetterStreet + oneLetterStreet
                            + oneLetterStreet;
                    //U.log("oneLetterStreet:"+oneLetterStreet);
                    address = address.replaceFirst(" " + oneLetterStreet + " ", " " + sixLetterStreet + " ");
                }
            }
        }
    ...
    private static String correctNumbersDisguisedAsWords(String address)
    {
        ...
        {
            String[] num2Words = { "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten" };
        ...
        {
            String[] stNum2Words = { "zeroeth", "first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "nineth",
                    "tenth", "eleventh", "twelth", "thirteenth", "fifteenth", "sixteenth", "seventeenth", "eighteenth","SEVENTEENTH"};
            for (int i = 0; i < stNum2Words.length; i++)
            {
                if (address.contains(stNum2Words[i]))
                {
                    //U.log(stNum2Words[i] + " " + "            " + i + _getNumSuf(i) + " ");
                    address = address.replaceFirst(stNum2Words[i], i + _getNumSuf(i));
                    break;
                }

                /*
                //U.log(i +"] correctNumbersDisguisedAsWords address:"+ address + "  stNum2Words[i]:"+stNum2Words[i]);
                //U.log(address.contains( " "+stNum2Words[i] ));
                if (address.matches(".+" + stNum2Words[i] + " (st|street|ave|road|rd|dr|drive|pl|place)"))
                {
    ...
    public static InputStream getInputStream(String url) throws Exception
    {
        ...
        hc.setRequestProperty("User-Agent", "Mozilla/5.0 (X11; Ubuntu; Linux i686; rv:13.0) Gecko/20100101 Firefox/13.0.1");
...
//    public static BufferedWriter output; = new BufferedWriter(new FileWriter(file));
    public static void writeFile(String text) throws IOException{
    	PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("SAC-Latency.txt", true)));
        out.println("\n"+text);
        out.close();
//     output.write(text);
//    output.close();
