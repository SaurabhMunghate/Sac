...
import com.shatam.shatamindex.search.FieldCache.CacheEntry;
...
public final class FieldCacheSanityChecker {
	...
	public Insanity[] check(CacheEntry... cacheEntries) {
		...
		final MapOfSets<Integer, CacheEntry> valIdToItems = new MapOfSets<Integer, CacheEntry>(
				new HashMap<Integer, Set<CacheEntry>>(17));

		final MapOfSets<ReaderField, Integer> readerFieldToValIds = new MapOfSets<ReaderField, Integer>(
		...
		final List<Insanity> insanity = new ArrayList<Insanity>(
				valMismatchKeys.size() * 3);

		insanity.addAll(checkValueMismatch(valIdToItems, readerFieldToValIds,
				valMismatchKeys));
	...
	private Collection<Insanity> checkValueMismatch(
			MapOfSets<Integer, CacheEntry> valIdToItems,
		...
		if (!valMismatchKeys.isEmpty()) {
			...
			final Map<ReaderField, Set<Integer>> rfMap = readerFieldToValIds
					.getMap();
			final Map<Integer, Set<CacheEntry>> valMap = valIdToItems.getMap();
			for (final ReaderField rf : valMismatchKeys) {
				final List<CacheEntry> badEntries = new ArrayList<CacheEntry>(
						valMismatchKeys.size() * 2);
				...
				insanity.add(new Insanity(InsanityType.VALUEMISMATCH,
						"Multiple distinct value objects for " + rf.toString(),
						badness));
			}
		}
		return insanity;
	...
	private Collection<Insanity> checkSubreaders(
		...
		Map<ReaderField, Set<ReaderField>> badChildren = new HashMap<ReaderField, Set<ReaderField>>(
				17);
		MapOfSets<ReaderField, ReaderField> badKids = new MapOfSets<ReaderField, ReaderField>(
				badChildren);

		Map<Integer, Set<CacheEntry>> viToItemSets = valIdToItems.getMap();
		Map<ReaderField, Set<Integer>> rfToValIdSets = readerFieldToValIds
				.getMap();
		...
		for (final ReaderField parent : badChildren.keySet()) {
			...
			List<CacheEntry> badEntries = new ArrayList<CacheEntry>(
					kids.size() * 2);
			...
			insanity.add(new Insanity(InsanityType.SUBREADER,
					"Found caches for descendants of " + parent.toString(),
					badness));
		}
	...
	private List<Object> getAllDescendantReaderKeys(Object seed) {
		...
		for (int i = 0; i < all.size(); i++) {
			...
			if (obj instanceof IndexReader) {
				IndexReader[] subs = ((IndexReader) obj)
	...
	private final static class ReaderField {
		...
		public boolean equals(Object that) {
			...
			return (this.readerKey == other.readerKey && this.fieldName
	...
	public final static class InsanityType {
		...
		public final static InsanityType SUBREADER = new InsanityType(
				"SUBREADER");

		public final static InsanityType VALUEMISMATCH = new InsanityType(
				"VALUEMISMATCH");

		public final static InsanityType EXPECTED = new InsanityType("EXPECTED");
	}
