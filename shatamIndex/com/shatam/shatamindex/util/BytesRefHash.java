...
import static com.shatam.shatamindex.util.ByteBlockPool.BYTE_BLOCK_MASK;
import static com.shatam.shatamindex.util.ByteBlockPool.BYTE_BLOCK_SHIFT;
import static com.shatam.shatamindex.util.ByteBlockPool.BYTE_BLOCK_SIZE;

import java.util.Arrays;
import java.util.Comparator;
import java.util.concurrent.atomic.AtomicLong;

import com.shatam.shatamindex.util.ByteBlockPool.DirectAllocator;
...
public final class BytesRefHash {
	...
	public BytesRefHash(ByteBlockPool pool, int capacity,
		...
		bytesUsed = bytesStartArray.bytesUsed() == null ? new AtomicLong(0)
	...
	public BytesRef get(int ord, BytesRef ref) {
		...
		assert ord < bytesStart.length : "ord exceeds byteStart len: "
				+ bytesStart.length;
	...
	public int add(BytesRef bytes, int code) {
		...
		if (e == -1) {
			...
			if (len2 + pool.byteUpto > BYTE_BLOCK_SIZE) {
				if (len2 > BYTE_BLOCK_SIZE) {
					throw new MaxBytesLengthExceededException(
							"bytes can be at most " + (BYTE_BLOCK_SIZE - 2)
									+ " in length; got " + bytes.length);
				}
				pool.nextBuffer();
			}
			...
			if (count >= bytesStart.length) {
				...
				assert count < bytesStart.length + 1 : "count: " + count
						+ " len: " + bytesStart.length;
			}
			e = count++;
			...
			if (length < 128) {
				...
				System.arraycopy(bytes.bytes, bytes.offset, buffer,
						bufferUpto + 1, length);
			} else {
			...
			if (e0 != -1) {
				...
				if (hashOnData) {
					...
					if ((bytes[start] & 0x80) == 0) {
						...
						len = (bytes[start] & 0x7f)
								+ ((bytes[start + 1] & 0xff) << 7);
						pos = start + 2;
					}
	...
	public static class MaxBytesLengthExceededException extends
			RuntimeException {
	...
	public static class TrackingDirectBytesStartArray extends BytesStartArray {
		...
		public int[] grow() {
			...
			bytesUsed.addAndGet((bytesStart.length - oldSize)
					* RamUsageEstimator.NUM_BYTES_INT);
			return bytesStart;
		}
	...
	public static class DirectBytesStartArray extends BytesStartArray {
		...
		public int[] grow() {
			assert bytesStart != null;
			return bytesStart = ArrayUtil.grow(bytesStart,
					bytesStart.length + 1);
		}
