...
import com.shatam.shatamindex.util.NamedThreadFactory;
...
public class ParallelMultiSearcher extends MultiSearcher {
	...
	public ParallelMultiSearcher(Searchable... searchables) throws IOException {
		this(Executors.newCachedThreadPool(new NamedThreadFactory(
				ParallelMultiSearcher.class.getSimpleName())), searchables);
	}

	public ParallelMultiSearcher(ExecutorService executor,
			Searchable... searchables) throws IOException {
		super(searchables);
	...
	public TopDocs search(Weight weight, Filter filter, int nDocs)
		...
		for (int i = 0; i < searchables.length; i++) {
			runner.submit(new MultiSearcherCallableNoSort(lock, searchables[i],
					weight, filter, nDocs, hq, i, starts));
		}
	...
	public TopFieldDocs search(Weight weight, Filter filter, int nDocs,
		...
		for (int i = 0; i < searchables.length; i++) {
			runner.submit(new MultiSearcherCallableWithSort(lock,
					searchables[i], weight, filter, nDocs, hq, sort, i, starts));
		}
		int totalHits = 0;
		float maxScore = Float.NEGATIVE_INFINITY;
	...
	@Override
	public void search(final Weight weight, final Filter filter,
	...
	@Override
	HashMap<Term, Integer> createDocFrequencyMap(Set<Term> terms)
			throws IOException {
		final Term[] allTermsArray = terms.toArray(new Term[terms.size()]);
		final int[] aggregatedDocFreqs = new int[terms.size()];
		final ExecutionHelper<int[]> runner = new ExecutionHelper<int[]>(
				executor);
		for (Searchable searchable : searchables) {
			runner.submit(new DocumentFrequencyCallable(searchable,
					allTermsArray));
		}
	...
	private static final class DocumentFrequencyCallable implements
			Callable<int[]> {
