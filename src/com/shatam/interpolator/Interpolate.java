...
public class Interpolate {
	...
	public static int distanceBetweenHouseNums(String sHouseNumber,
...

		{
			...
			if (lfrom != -1 && lfrom % 2 == hn % 2) {
				...
				return isBetween(hn, lfrom, lto) ? 0 : Math.min(
						Math.abs(hn - lfrom), Math.abs(hn - lto));
			}
		}

		{
			...
			if (rfrom != -1 && rfrom % 2 == hn % 2) {
				...
				int ret = isBetween(hn, rfrom, rto) ? 0 : Math.min(
						Math.abs(hn - rfrom), Math.abs(hn - rto));
	...
	public static Coordinate houseNumberOnStreetSegments(Coordinate[] vertices,
			int leftFrom, int leftTo, int rightFrom, int rightTo,
			int targetHouseNumber) {
		Coordinate target = null;

		double targetPercentOfTotalLength = 100
				* (targetHouseNumber - leftFrom) / (leftTo - leftFrom);
		...
		if (targetPercentOfTotalLength <= 0
				|| targetPercentOfTotalLength >= 100) {
			Coordinate A = vertices[0];
			Coordinate B = vertices[vertices.length - 1];
			StreetSegment seg = new StreetSegment(A, B);
			double needToExtendBy = totalLength * targetPercentOfTotalLength
					/ 100;
	...
	private static Coordinate pointOnSegment(StreetSegment seg,
		...
		return new Coordinate(seg.getFrom().x + targetDx, seg.getFrom().y
				+ targetDy);
	}

	public static void findPoint(AddressStruct addStruct) throws Exception {
		WKTReader wkt = new WKTReader();
		MultiLineString geo = (MultiLineString) wkt.read(addStruct
				.get(AddColumns.GEO));
		...
		if (StrUtil.isEmpty(addStruct.getHouseNumber())) {
			...
			foundPt = houseNumberOnStreetSegments(vertices, lfrom, lto, rfrom,
					rto, hn);
