...
public class AbbrReplacement
{
    ...
    private static void addAbbr(String abbrev, String full, int is_es, int pref, int suff)
    {
        addAbbr(abbrev, full, is_es, pref, suff, NONSTANDARD_ABBR);
    }

    private static void addAbbr(String abbrev, String full, int is_es, int pref, int suff, int abbrType)
    {
    ...
    static
    {
       ...
       // addAbbr("Ave", "Avenida", SPANISH, 1, 0, STANDARD_ABBR);
        ...
        addAbbr("BIA Highway", "Bureau of Indian Affairs Highway", ENGLISH, 1, 1);
        addAbbr("BIA Highway", "Bureau of Indian Affairs Highway", SPANISH, 1, 0);
        ...
        addAbbr("Nat For Dev Rd", "National Forest Development Road", ENGLISH, 1, 1);
        ...
        addAbbr("Nat For Development", "National Forest Development", ENGLISH, 1, 1);
        addAbbr("Natl Forest Develop", "National Forest Development", ENGLISH, 1, 1);
        ...
        addAbbr("North F Dev Rd", "National Forest Development Road", ENGLISH, 1, 1);
        ...
        // addAbbr ( "St Market", "State Market", 0, 1, 1 ); ####
        // addAbbr ( "St Line", "State Line", 0, 1, 1 ); ####
        // addAbbr ( "Saint Line", "State Line", 0, 1, 1 ); ####
        // addAbbr ( "Street Line", "State Line", 0, 1, 1 ); ####
        // addAbbr ( "St", "Saint", 0, 1, 0 ); ####
        // addAbbr ( "St Andrews", "Saint Andrew's", 0, 1, 1 ); ####
        // addAbbr ( "St Johns", "Saint John's", 0, 1, 1 ); ####
        // addAbbr ( "St Marys", "Saint Mary's", 0, 1, 1 ); ####
        // addAbbr ( "St Matthews", "Saint Matthew's", 0, 1, 1 ); ####
        // addAbbr ( "St Josephs", "Saint Joseph's", 0, 1, 1 ); ####
        // addAbbr ( "St Michaels", "Saint Michael's", 0, 1, 1 ); ####
        // addAbbr ( "St Bernards", "Saint Bernard's", 0, 1, 1 ); ####
        // addAbbr ( "St Helens", "Saint Helen's", 0, 1, 1 ); ####
        // addAbbr ( "St Annes", "Saint Anne's", 0, 1, 1 ); ####
        // addAbbr ( "St Albans", "Saint Alban's", 0, 1, 1 ); ####
        // addAbbr ( "St Martins", "Saint Martin's", 0, 1, 1 ); ####
        // addAbbr ( "St Pauls", "Saint Paul's", 0, 1, 1 ); ####
        // addAbbr ( "Saint Andrews", "Saint Andrew's", 0, 1, 1 ); ####
        // addAbbr ( "Saint Johns", "Saint John's", 0, 1, 1 ); ####
        ...
        //addAbbr("Wy", "Way", ENGLISH, 0, 1, NONSTANDARD_ABBR);
        ...
        //addAbbr("RR", "RURAL ROUTE", ENGLISH, 1, 1);

        
        //standardFull2AbbrMap.put("highway", "highway");
    }
    ...
    public static String getFullName(String input, int where, String state) throws Exception
    {
        input = input.trim().toLowerCase();
    ...
    public static String replaceLargePhrases(String input, String state) throws Exception
    {
        input = input.trim().toLowerCase();
        ArrayList<String[]> abbrArr = isSpanish(state) ? _SpanishAbbrWithSpaces : _EnglishEbbrWithSpaces;

        for (String[] arr : abbrArr)
        {
            //U.log("A replaceLargePhrases input:" + input + "  arr[0]:"+arr[0] + "  arr[1]:"+arr[1]);
            if (input.contains(arr[0]))
            {
           //     U.log("B replaceLargePhrases input:" + input);
                input = input.replace(arr[0], arr[1]);
           //     U.log("C replaceLargePhrases input:" + input);
            }
        }
        return input.trim();
    ...
    public static String getFullName(String input, String state) throws Exception
    {
        input = input.trim().toLowerCase();
    ...
    public static String getAbbr(String input, String state){
        ...
        if (StrUtil.isEmpty(v) || v.equals(input)) v=  getAbbr( input, PREFIX, state);
        return v;
    }
    ...
    public static String getFullAddress(String inputAddress, String state)
    {
        ...
        for (String a : inputAddress.split(" "))
        {
            ...
            String abr = (suffixFull2AbbrMap.containsKey(a))? suffixFull2AbbrMap.get(a): null;
            if (StrUtil.isEmpty(abr))
            {
                abr =(priffixFull2AbbrMap.containsKey(a))? priffixFull2AbbrMap.get(a): null;
            }
            /*
            String abr = AbbrReplacement.getAbbr(a, PREFIX, state);
            if (StrUtil.isEmpty(abr) || a.equals(abr))
                abr = AbbrReplacement.getAbbr(a, SUFFIX, state);
            */
    ...
    public static String abbrFullAddress(String inputAddress, String state)
    {
        ...
        for (String a : inputAddress.split(" "))
        {
            String abr = (standardFull2AbbrMap.containsKey(a))? standardFull2AbbrMap.get(a): null;
    ...
    /*
     * public static String getUnitAbbr(String input) { HashMap<String, String>
     * unitTypeMap = new HashMap<>(); unitTypeMap.put("apartment", "apt");
     * unitTypeMap.put("building", "bldg"); unitTypeMap.put("floor", "fl");
     * unitTypeMap.put("suite", "ste"); unitTypeMap.put("unit", "unit");
     * unitTypeMap.put("room", "rm"); unitTypeMap.put("department", "dept");
     * unitTypeMap.put("po box", "box");
     * 
     * input = input.trim().toLowerCase();
     * 
     * if (unitTypeMap.containsKey(input)) { return unitTypeMap.get(input); }
     * else { return input;
     * 
     * }
     * 
